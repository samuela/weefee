...
use nmdbus::{NetworkManager, accesspoint::AccessPoint, device::Device, device_wireless::DeviceWireless};
...
impl NetworkClient {
  ...
  fn get_saved_connections(&self) -> Result<Vec<SavedConnection>> {
    let settings_proxy = self
      .connection
      .with_proxy(NM_BUS_NAME, NM_SETTINGS_PATH, Duration::from_secs(5));
    ...
    for conn_path in connection_paths {
      let conn_proxy = self
        .connection
        .with_proxy(NM_BUS_NAME, &conn_path, Duration::from_secs(5));

      // Get connection settings
      let (settings,): (HashMap<String, PropMap>,) =
        match conn_proxy.method_call("org.freedesktop.NetworkManager.Settings.Connection", "GetSettings", ()) {
          Ok(s) => s,
          Err(_) => continue,
        };
              ...
              if let Some(wifi_settings) = settings.get("802-11-wireless") {
                if let Some(ssid_variant) = wifi_settings.get("ssid") {
                  if let Some(ssid_bytes) = ssid_variant.0.as_iter() {
                    let ssid_vec: Vec<u8> = ssid_bytes.filter_map(|v| v.as_u64().map(|b| b as u8)).collect();
  ...
  pub fn get_device_info(&self) -> Result<WifiDeviceInfo> {
    let nm_proxy = self.connection.with_proxy(NM_BUS_NAME, NM_PATH, Duration::from_secs(5));
    ...
    for dev_path in device_paths {
      ...
      if let Ok(dev_type) = dev_proxy.device_type() {
        if dev_type == DEVICE_TYPE_WIFI {
          ...
          let options: HashMap<String, Variant<Box<dyn dbus::arg::RefArg>>> = HashMap::new();
          ...
          if let Ok(aps) = dev_proxy.get_access_points() {
            ...
            for ap_path in aps {
              let ap_proxy = self
                .connection
                .with_proxy(NM_BUS_NAME, &ap_path, Duration::from_secs(5));
  ...
  pub fn connect(&self, ssid: &str, password: &str) -> Result<()> {
    ...
    let active_conn_path = if let Some(conn) = saved_conn {
      ...
      connection_settings.insert("type".to_string(), Variant(Box::new("802-11-wireless".to_string())));

      let mut wifi_settings: PropMap = HashMap::new();
      wifi_settings.insert("ssid".to_string(), Variant(Box::new(ssid.as_bytes().to_vec())));
      wifi_settings.insert("mode".to_string(), Variant(Box::new("infrastructure".to_string())));
      ...
      if !password.is_empty() {
        ...
        security_settings.insert("key-mgmt".to_string(), Variant(Box::new("wpa-psk".to_string())));
      ...
      let (connection_path, active_path) =
        match nm_proxy.add_and_activate_connection(final_map, wifi_device_path, specific_object) {
          Ok(result) => result,
          Err(e) => {
            // If this fails immediately, it could be due to secrets issues or invalid params
            let err_str = e.to_string();
            if err_str.contains("secrets") || err_str.contains("802-1x") || err_str.contains("password") {
      ...
      let wait_result = self.wait_for_connection_state(&active_path, new_connection_path.as_ref());
  ...
  fn wait_for_connection_state(
    ...
    loop {
      ...
      let ac_proxy = self
        .connection
        .with_proxy(NM_BUS_NAME, active_conn_path.clone(), Duration::from_secs(5));
        ...
        Ok(state) => {
          match state {
            ...
            NM_ACTIVE_CONNECTION_STATE_DEACTIVATED => {
              ...
              if let Ok(reason) = ac_proxy.get::<u32>("org.freedesktop.NetworkManager.Connection.Active", "StateReason")
              {
                // Common reasons:
                ...
                match reason {
                  ...
                  _ => {
                    return Err(anyhow::anyhow!("Connection failed (reason: {})", reason));
                  }
                }
              }
  ...
  fn delete_connection(&self, connection_path: &dbus::Path) -> Result<()> {
    let conn_proxy = self
      .connection
      .with_proxy(NM_BUS_NAME, connection_path, Duration::from_secs(5));

    conn_proxy
      .method_call::<(), _, _, _>("org.freedesktop.NetworkManager.Settings.Connection", "Delete", ())
  ...
  pub fn forget_network(&self, ssid: &str) -> Result<()> {
    ...
    let connections_to_delete: Vec<_> = saved_connections.iter().filter(|c| c.ssid == ssid).collect();
